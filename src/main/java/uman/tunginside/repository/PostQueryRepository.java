...
public class PostQueryRepository {
    ...
    public List<PostSummaryDTO> findByCondition(String abbr, Integer page, Integer likeCut, String search) {
        return queryFactory.select(Projections.constructor(PostSummaryDTO.class, post.id, category.name, member.nickname, post.title, post.ip_addr, post.create_at, post.last_modified_at, post.post_like_count, post.comment_count))
                .from(post)
    ...
    public Optional<PostDetailDTO> findDetailById(Long postId) {
        return em.createQuery("select p.id, c.name, c.abbr, m.nickname, p.ip_addr, p.last_modified_ip, p.create_at, p.last_modified_at, p.title, p.content, p.post_like_count, p.post_dislike_count, p.comment_count from Post p join p.category c on p.id = :postId left join p.member m", PostDetailDTO.class)
